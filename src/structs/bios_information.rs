...
mod tests {
    ...
    fn test_bios_information() {
        let bios_information_bytes = vec![
            ...
            0x01, 0x02, 0x00, 0x00, 0x03, 0xFF, 0x80, 0x18, 0x19, 0x0C, 0x00, 0x00, 0x00, 0x00, 0x03, 0x0D, 0xFF, 0xFF, 0xFF, 0xFF, 0x10, 0x00, 
        ...
        // basic field tests
        assert_eq!(bios_information.vendor().expect("vendor field exists"), "Microsoft Corporation".to_string());
        assert_eq!(bios_information.version().expect("version field exists"), "1.2547.140".to_string());
        assert_eq!(bios_information.starting_address_segment().expect("starting_address_segment field exists"), 0);
        assert_eq!(bios_information.release_date().expect("release_date field exists"), "09/14/2020".to_string());
        assert_eq!(bios_information.rom_size().expect("rom_size field exists"), 0xFF);
        assert_eq!(bios_information.characteristics().expect("characteristics field exists"), 202971264);
        assert_eq!(bios_information.bios_vendor_reserved_characteristics().expect("bios_vendor_reserved_characteristics field exists"), 0);
        assert_eq!(bios_information.system_vendor_reserved_characteristics().expect("system_vendor_reserved_characteristics field exists"), 0);
        assert_eq!(bios_information.characteristics_extension0().expect("characteristics_extension0 field exists"), 3);
        assert_eq!(bios_information.characteristics_extension1().expect("characteristics_extension1 field exists"), 13);
        assert_eq!(bios_information.system_bios_major_release().expect("system_bios_major_release field exists"), 255);
        assert_eq!(bios_information.system_bios_minor_release().expect("system_bios_minor_release field exists"), 255);
        assert_eq!(bios_information.e_c_firmware_major_release().expect("e_c_firmware_major_release field exists"), 255);
        assert_eq!(bios_information.e_c_firmware_minor_release().expect("e_c_firmware_minor_release field exists"), 255);
        assert_eq!(bios_information.extended_rom_size().expect("extended_rom_size field exists"), 16);
